...
def load_my_data(chosen_repr, config):
    ...
    df1 = pd.read_hdf(os.path.join('..','data','df_train_'+ str(chosen_repr) +'_norm.h5'),'df')
    df2 = pd.read_hdf(os.path.join('..','data','df_valid_norm.h5'),'df')
...
def load_test_multitap(chosen_repr):
    ...
    k_range = [3,6,9,12,15]
    df_all = pd.read_hdf(os.path.join('..','data','df_test_repr3_0_norm.h5'),'df')
    ...
    for k in k_range:
        df_k = pd.read_hdf(os.path.join('..','..','jupyter','data','df_test_repr3_'+ str(k) +'_norm.h5'),'df')
...
def load_my_data_multitap(chosen_repr, config):
    ...
    # datasets saved before (problems with library and python versions compatibility. Python 3.4 needed for library libtfr)
    df1 = pd.read_hdf(os.path.join('..','..','jupyter','data','df_train_multitaper_norm.h5'),'df')
    df2 = pd.read_hdf(os.path.join('..','..','jupyter','data','df_valid_multitaper_norm.h5'),'df')
