...
#%% Parameters to set
chosen_repr = '8_classic_plus_MIR' # representation name, for which we want to check best models. 
...
df1 = pd.read_hdf(os.path.join('df_train_'+ str(chosen_repr) +'_norm.h5'),'repr5')
df2 = pd.read_hdf(os.path.join('df_valid_norm.h5'),'df') # valid is smaller set, so all representations are kept in one file
all_repr = cfg.representation_1d + cfg.representation_2d
...
file_names, indices, info_chunksy, repr_full_scaled = function_representations_all(cfg.path_test1618_wav, data_test_new, if_scaler, \
...
for model_type in range(0,len(resultsy_names)):
    ...
    saving_dir_folder_model = os.path.join('..','results','6h-PR','charts_data_6_5', chosen_repr, classifier)
    try:
...
testing_target = np.array(pd.read_csv(os.path.join(saving_dir_folder,'nb','8_classic_plus_MIR-nb-665-test_new.predictions'),sep='\t')['has_bird'])
