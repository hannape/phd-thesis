...
def function_representations_all2(path_wav, data_settype, if_scaler, repr_1d_summary, summary_1d, sr, chunk_length_ms, \
                          n_fft, win_length, hop_length, window, f_min, f_max, n_mels, N, step, Np, K, tm, flock, tlock, \
                          chosen_repr = '', nr_rec_start=0, rec_batch_size=18):
  ...
  Args: 
    ...
    last two parameters - batches for reading long recordings, so that there's no "run of memory' error
  Returns:
   ...
   Four representations, without multitaper repr3 (problems with libraries and python versioning, done in a separate script. Repr3 commented here)   
  ''' 
  ...
  repr_full1, repr_full2= [[] for _ in range(np.shape(file_names_set)[0])],[[] for _ in range(np.shape(file_names_set)[0])]
  ...
  repr_full= [[] for _ in range(np.shape(file_names_set)[0])]
  
  print("Recordings in a set: ",file_names_set) 

  scaler_path = os.path.join(cfg.main_folder_path, '_1_Create_DF_representations','scaler')
  scaler1 = joblib.load(os.path.join(scaler_path,'scaler_spektrogram')) if if_scaler==True else None  
  scaler2 = joblib.load(os.path.join(scaler_path,'scaler_mel_spektrogram')) if if_scaler==True else None  
  #scaler3 = joblib.load(os.path.join(scaler_path,'scaler_multitaper') if if_scaler==True else None 
  scaler4 = joblib.load(os.path.join(scaler_path,'scaler_8_classic')) if if_scaler==True else None  
  scaler5 = joblib.load(os.path.join(scaler_path,'scaler_8_classic_plus_MIR')) if if_scaler==True else None  
  
  if len(chosen_repr)==0:  # no chosen represenation, all representations calculated
    ...
    for k in range(0,np.shape(file_names_set)[0]): 
        ...
        if not any(df_to_np[k][4]) and not any(df_to_np[k][5]):  
          ...
          print(np.shape(info_chunksy[k][5]), 'no calls marked')
    ...
    print('One representation, ', chosen_repr)  
    for k in range(nr_rec_start, nr_rec_start + rec_batch_size): 
    ...
    return [file_names_set , indices, info_chunksy, repr_full] 
