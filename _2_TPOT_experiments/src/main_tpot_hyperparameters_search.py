...
import grid  
from config_tpot import parse_model_config, parse_tpot_config, parse_task_config, utils_section  # _2_TPOT_experiments.src.
...
''' Training and checking models from chosen families, with different hyperparameters, during specified time. Input data transformed to chosen representation.
 ...
 Running script: python script_name model_config tpot_config metrics_congfig representation_type saving_dir 
   e.g. python main_tpot_hyperparameters_search.py ..\configs\trees.cfg ..\configs\tpot-mini.cfg ..\configs\metrics.cfg 8_classic ..\results
...
if __name__=='__main__':
    ...
    exp_dir = os.path.join(sys.argv[-1],"PR", f"{chosen_repr}-{model_cfg[utils_section]['model']}", time.strftime('%Y-%m-%d-%H-%M'))
